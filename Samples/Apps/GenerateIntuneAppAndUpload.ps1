...
$TenantID = "stctravel.onmicrosoft.com"

<#
$modules = @("IntuneWin32App", "AzureAD", "Microsoft.Graph")
...
########################Start Script################
$passwordFile = Join-Path $parentPath "Password.xml"
...
if(Test-Path -Path $passwordFile){
  ...
  $Credential | Export-Clixml $passwordFile
}


#############################################################
$Credential = $null
$Credential = Import-Clixml $passwordFile
##################Connect to Azure####################
...
try 
{
  ...
  Write-Host "loaded.... AzureAD Module"
}
catch 
{
